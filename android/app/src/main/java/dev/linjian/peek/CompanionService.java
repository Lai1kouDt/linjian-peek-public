...
public class CompanionService extends Service {
    ...
    private static volatile long lastSuccessfulPollMs = 0L;
    private static volatile int consecutivePollFailures = 0;
    private static volatile String lastPollError = "";
    ...
    public static long lastSuccessfulPollMs() { return lastSuccessfulPollMs; }
    public static int consecutivePollFailures() { return consecutivePollFailures; }
    public static String lastPollError() { return lastPollError; }
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        DebugState.append(this, "掌心窗 Lite " + AppPrefs.APP_VERSION_NAME + " 服务已启动，目标：" + serverUrl);
    ...
    private void pollLoop() {
        ...
        try { uploadStateThrottled(serverUrl, token, this, false); }
        catch (Exception e) { DebugState.append(this, "状态上报异常：" + ScreenshotService.shortMsg(e)); }
        try {
            ...
            lastPollError = ScreenshotService.shortMsg(e);
            DebugState.append(this, "轮询异常：" + lastPollError);
        }
    ...
    public static void handleCommandBody(Context ctx, String body, String serverUrl, String token) {
        try {
            ...
            if (raw instanceof String) {
                DebugState.append(ctx, "Lite 已拒绝旧式字符串命令：" + raw);
                return;
            }
            JSONObject cmd = (JSONObject) raw;
            String id = cmd.optString("id", "");
            String action = cmd.optString("action", "noop");
            if (!isLiteCommandAction(action)) {
                String result = "unsupported_in_lite:" + action;
                DebugState.append(ctx, "Lite 已拒绝非核心命令：" + action);
                try { reportCommand(ctx, serverUrl, token, id, false, result); } catch (Exception ignored) { }
                return;
            }
            ...
            if ("set_alarm".equals(action)) {
                JSONObject rr = AlarmBridge.handle(ctx, cmd);
                boolean ok = rr.optBoolean("ok", false);
                DebugState.append(ctx, "执行闹钟命令：" + rr.toString());
                try { reportCommand(ctx, serverUrl, token, id, ok, rr.toString()); uploadStateThrottled(serverUrl, token, ctx, true); } catch (Exception ignored) { }
                return;
            }
    ...
    private static boolean isLiteCommandAction(String action) {
        return "set_alarm".equals(action)
                || "screen_off".equals(action)
                || "turn_screen_off".equals(action)
                || "lock_screen".equals(action)
                || "phone_screen_off".equals(action);
    }
    ...
    private static JSONObject performAction(Context ctx, String action, String app, String pkg, float x, float y, float x1, float y1, float x2, float y2, long duration, int hour, int minute, String title, String message, boolean vibrate, String serverUrl, String token, boolean skipUi, String targetText, String inputText, String match, int index, boolean append) {
        ...
        try {
            ...
            if ("wait".equals(action)) { ok = true; result = "wait";
            ...
            } else if ("screen_off".equals(action) || "turn_screen_off".equals(action) || "lock_screen".equals(action) || "phone_screen_off".equals(action)) {
                boolean requested = svc != null && svc.doLockScreen();
                if (requested) try { Thread.sleep(650L); } catch (InterruptedException interrupted) { Thread.currentThread().interrupt(); }
                ...
                ok = requested && !screenOn;
                JSONObject lock = new JSONObject();
                lock.put("lock_requested", requested);
                lock.put("lock_success", ok);
                lock.put("screen_on", screenOn);
                lock.put("screen_off", !screenOn);
                lock.put("confirmed", ok);
                lock.put("verified_at_ms", System.currentTimeMillis());
                if (!requested) lock.put("error", "accessibility_missing_or_android_too_old");
                else if (screenOn) lock.put("error", "lock_requested_but_screen_still_on");
            ...
            } else if ("set_alarm".equals(action)) { JSONObject alarm = AlarmBridge.handle(ctx, new JSONObject().put("operation", "set").put("hour", hour).put("minute", minute).put("message", message).put("vibrate", vibrate).put("skip_ui", skipUi)); ok = alarm.optBoolean("ok", false); result = alarm.toString();
    ...
    private static void uploadStateThrottled(String serverUrl, String token, Context ctx, boolean force) throws Exception {
        ...
        JSONObject state = LiteState.collect(ctx);
