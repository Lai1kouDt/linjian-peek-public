...
> Render 免费版说明：Blueprint 会把 `zhangxinchuang-server` 的公网主机名自动交给 MCP，并使用 HTTPS 连接。不要改回 `host:10000` 私网地址；免费 Web Service 无法接收这类私网流量。手机数据和控制接口仍由 `LINJIAN_TOKEN` 保护。
